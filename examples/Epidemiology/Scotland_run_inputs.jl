...
function run_model(times::Unitful.Time, interval::Unitful.Time, timestep::Unitful.Time; do_plot::Bool = false, do_download::Bool = true, save::Bool = false, savepath::String = pwd())
    ...
    birth_rates[:, 2:4] .= uconvert(day^-1, 1/paramraw[3]years)
    death_rates[1:end-1, :] .= uconvert(day^-1, 1/paramraw[4]years)
    virus_growth_asymp = virus_growth_presymp = virus_growth_symp = fill(paramraw[5]/day, age_categories)
    virus_decay = paramraw[6]/day
    beta_force = fill(paramraw[2]/day, age_categories)
    beta_env = fill(paramraw[2]/day, age_categories)
