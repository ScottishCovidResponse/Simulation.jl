...
params = (
	...
	method = :gillespie,  # :gillespie or :tau (stoch model)
	num_itns = 5,  # iteration
	num_recs = 301, # records per iteration
	num_groups = 1, # metapopulations
	contact_rate = 1.0, # contact rate (IBM)
...
# Run simulation
times = 151days; interval = 1day; timestep = 1day
abuns = zeros(Int64, numclasses, prod(grid), div(times, interval) + 1)
@time simulate_record!(abuns, epi, times, interval, timestep; save=false, save_path=".")
